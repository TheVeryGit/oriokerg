...
> модификаторы прозрачности (`bg-card/70`, `bg-surface-2/50` и т.п.). В сыром CSS используй
> `rgb(var(--token))`, НЕ `var(--token)` напрямую (иначе цвет «пропадёт»). Новые цвет-токены
> добавляй тоже в канальной форме.
...
> Фото котят — **реальные** (загружены владельцем через админку, лежат в
> `public/images/uploads/`). Фото из карточек автоматически попадают в галерею главной.
> Коллекция `content/cats/` пока пустая — кошки-производители ещё не заведены.
